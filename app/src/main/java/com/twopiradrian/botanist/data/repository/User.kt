...
class UserRepository: UserRepository {
    ...
    override suspend fun getFeed(token: String, request: GetFeed.Request): GetFeed.Response {
        return api.getFeed(token, request.page, request.pageSize)
    }

    override suspend fun getProfile(token: String, request: GetProfile.Request): GetProfile.Response {
        return api.getProfile(token, request)
    }
